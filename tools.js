...
function mergeNode(files, exportModules) {
  ...
  // 找出根节点
  console.log('find root file', mods);
    ...
    while (list.length) {
      ...
      list.forEach(function (n) {
        n.reqs.forEach(function (m) {
          let next = mergeFromRoot(result[n.root], m, n.root, checkRoot);
          if (next) {
            tmp.push(next);
          }
        });
      ...
      list = tmp;
      // mergeFromRoot(null, root, root, checkRoot);
    }
  });
  ...
  function mergeFromRoot(arr, moduleName, root, check) {
    let tmp = fileMap[moduleName];
    if (!tmp) {
      return;
    }

    if (!arr || check(tmp.__roots) > 1) {
