    ...
    if (stat.isDirectory()) {
      ...
      try {
        ...
        if (cube.config.moduleMap && cube.config.moduleMap[modName]) {
          debug('>>>>>>>>>', cube.config.moduleMap, modName);
          modPath = path.join(modPath, cube.config.moduleMap[modName]);
        }
        /**
  ...
  if (/^@?\w+/.test(module)) {
    ...
    while (dir) {
      ...
      try {
        ...
        if (ifRootPath(dir)) {
          console.error('[CUBE_WARN] required node_module: `' + origModule + '` not found in file:', base);
          p = module;
          break;
        }
        dir = path.dirname(dir);
