  ...
  try {
    ...
    if (stat && stat.isDirectory()) {
      ...
      try {
         ...
         */
        tmpModPath = cube.config.moduleMap[modName];
         ...
         */
          ...
          debug('testModPath:folder, try to loading package.json:browserify', tmpModPath);
        }
        /**
         * then search for pkg.main
         */
        else {
          tmpModPath = path.join(modPath, pkg.main || 'index.js');
          debug('testModPath:folder, try to loading package.json:main ', tmpModPath);
        }
        ...
        debug('testModPath:folder, try to loading folder/index script', tmpModPath);
        try {
          tmpModPath = tryFiles(cube, root, tmpModPath);
        } catch (e) {
          debug('testModPath:folder/index not found, try itself: %s', tmpModPath);
          throw e;
        }
      }
    } else {
      debug('testModPath:file', modPath);
      tmpModPath = tryFiles(cube, root, modPath);
    }
  } catch (e) {
    debug('testModPath:finaly path not found', modPath);
    throw e;
  }
...
function resolvePath(cube, root, base, module) {
  ...
  if (/^@?\w+/.test(module)) {
    ...
    try {
      ...
      console.log('[CUBE_WARN] require path:', module, 'not found');
      p = module;
      STATE = 1;
    }
  }
...
exports.processScript = function (cube, data) {
  ...
  estraverse.traverse(ast, {
    enter: function (node) {
      ...
      var resolvedRes;
      resolvedRes = resolvePath(cube, root, filepath, v);
