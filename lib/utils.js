...
exports.fixProcessorConfig = function (cfg, root, noRelative) {
  if (!cfg) {
    return;
  }
  Object.keys(cfg).forEach((key) => {
    ...
    this.fixProcessorPath(procs, root, noRelative);
  });
};
 ...
 * 补全processor的路径
 * @param  {Path} root
 * @param  {Array} processors list of processor
 * @return {Array} processors
 */
exports.fixProcessorPath = function (processors, root, noRelative) {
  ...
  processors.forEach(function (v, i, a) {
    ...
    /**
     * build in processors, ignore
     */
    if (v[0].startsWith('BUILDIN_')) {
      return;
    }
    /**
     * if relative path
