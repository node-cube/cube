...
function tryFiles(cube, root, modPath) {
  ...
  if (targetExt === null) {
    var err = new Error('[ERROR] required module not found: `' + modPath.substr(root.length) + '`');
    err.possiblePath = modPath;
    throw err;
  }
      ...
      enter: function (node) {
        ...
        */
        if (node.type === syntax.CallExpression) {
          ...
          // filter other calls
          if (node.callee.name !== 'require' && node.callee.name !== 'async') {
            return;
          }
          //


          // check if require() is redefined in current scope
          ...
          do {
            for (var i = 0; i < tmpScope.variables.length; ++i) {
              ...
              if ((v.name === 'require' || v.name === 'async') && v.defs.length) {
                flagRedefined = true;
                break;
              }
            }
          ...
          // ignore empty require and async require or
          // arguments.length > 3
          // `async('module')` `async('css', namespace)`
          // `async('css', namespace, function (css) {})`
          ...
          try {
            ...
            console.log(err.message, 'using the origin path');
            abspath = abspathOrigin = err.possiblePath.substr(root.length);
          }
          if (remote) {
            abspath = remote + ':' + abspath;
          }
          // if callee.name === async, ignore
