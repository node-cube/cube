...
exports.genExportStmt = function (varList) {
  ...
  varList.forEach((varItem) => {
    ...
    if (varItem.source) {
      if (varItem.varValue) {
        let idf 
        if (t.isIdentifier(varItem.varValue)) {
          idf = varItem.varValue;
        } else {
          idf = t.identifier(varItem.varValue);
        }
...
exports.genExportNamedStmt = function (subNode) {
  ...
  } else if (t.isClassDeclaration(subNode.declaration)) {
    // export class A{}
    return [subNode, t.expressionStatement(t.assignmentExpression(
      '=',
      t.memberExpression(t.identifier('exports'), subNode.id),
  ...
  } else if (t.isTSEnumDeclaration(subNode)) {
    // enum -> {}
    let objDecl = t.variableDeclaration('const', [
      t.variableDeclarator(
        subNode.id,
        t.objectExpression(subNode.members.map((mb) => {
          return t.objectProperty(t.identifier(mb.id.name), mb.initializer)
        }))
      )
    ])
    return [objDecl, t.expressionStatement(t.assignmentExpression(
      '=',
      t.memberExpression(t.identifier('exports'), subNode.id),
      subNode.id
    ))]
  } else if (t.isTSInterfaceDeclaration(subNode)) {
