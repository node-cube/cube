...
const CubeTransformImport = require('./cube_transform_import');
...
module.exports = {
  ...
  processScript(data, callback) {
    ...
    // enable wrap plugin
    swcOptions.plugin = (m) => {
      let transformImport = new CubeTransformImport({
        cube: this,
        data: data,
      });
      data.asyncs = transformImport.loads;
      data.asyncsOrigin = transformImport.loadsOrigin;
      data.requires = transformImport.requires;
      data.requiresOrigin = transformImport.requiresOrigin;
      // data.debugInfo = t.debugInfo;
      return transformImport.visitProgram(m);
    }
