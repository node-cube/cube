...
module.exports = {
   ...
   */
  resolveModulePath(root, curModule, module, callback) {
    ...
    if (/^\w+:/.test(module)) {
      ...
      while (dir) {
        ...
        if (ignore.ignore) {
          console.error(`[CUBE_WARN] "${module}" which required by "${curModule}" is ignored by .cubeignore config`);
          p = module;
        ...
        try {
          p = testModPath(this, root, nodeModulePath, module);
          debug('resolvePath: ok > ', p);
          break;
        } catch (e) {
          debug('node_module not found:', path.join(dir, '/node_modules/'));
          if (ifRootPath(dir)) {
            console.error(`[CUBE_WARN] required node_module: "${module}" not found in file: ${curModule}`);
            p = module;
            STATE = 1;
            break;
          }
      ...
      try {
        p = testModPath(this, root, p, module);
      } catch (e) {
        debug('resolvePath error, module not found', module);
        console.log(`[CUBE_WARN] require path:', module, 'not found in file: ${curModule}`);
        p = module;
        STATE = 1;
      }
    }
  ...
  processScript(data, callback) {
    ...
    estraverse.traverse(ast, {
      enter: function (node) {
        ...
        let flagRedefined = false;
        let tmpScope = currentScope;
        let module;  // 在require中被引用的module
        // 检查是否被重新定义过，如果重新定义了 require 或 load, 就忽略引用关系
        do {
          for (var i = 0; i < tmpScope.variables.length; ++i) {
            module = tmpScope.variables[i];
            if (module.name === node.callee.name && module.defs.length) {
              flagRedefined = true;
              break;
            }
          }
        ...
        wait ++;
        self.resolveModulePath(root, filepath, module, function (err, resolveResult) {
          wait --;
          if (err) {
            return done(err);
          }
          ...
          var type = self.extMap[ext];
          // 名字转换，如果是release状态，需要将名字做转换, 类似: .less -> .less.js
          abspath = utils.moduleName(abspathOrigin, type, config.release);
          debug('module path resolved, origin:', module, 'absOrigin:', abspathOrigin, 'abs:', abspath);
  ...
  processStyle(data, callback) {
    ...
    if (flagCompress) {
      cssProcess.minify(data.code, function (err, minified) {
        if (err) {
  ...
  wrapScript(data, callback) {
    ...
    if (!data.ast) {
      try {
        data.ast = esprima.parse(data.code, {filename: finalQpath});
      } catch (e) {
        return callback(e);
      }
    }
    ...
    if (flagCompress) {
      if (!data.wrap) {
        data.code = compressedCode(finalQpath, data.ast);
      } else {
        ...
        wraper.body[0].expression.arguments[2].body.body = data.ast.body;
        data.ast.body.push(body[0]);
        var compressRes = compressedCode(finalQpath, wraper);
        data.codeWraped = compressRes[0];
      }
    } else {
      data.codeWraped = genUncompressCode(
        header,
        data.code,
        footer,
        this.config.release
  ...
  wrapTemplate(data, callback) {
    ...
    if (flagCompress) {
      data.codeWraped = compressedCode(qpath, wrapedCode);
    } else {
      data.codeWraped = wrapedCode;
    }
    data.queryPath = finalQpath;
    callback(null, data);
  }
