...
function testModPathBaisc(cube, modPath, modName, modRootPath, isDirectory) {
  ...
  const rootPath = /^@?\w+/.test(modName) ? path.resolve(modRootPath, 'node_modules') : modRootPath;
  try {
    let pkg;
    let isRoot = false;
    // 获取 package.json
     ...
     * }
     */
    else if (pkg && pkg.browser && typeof pkg.browser === 'object' && !!exist(pkg.browser, modPath, rootPath)) {
  ...
  resolveModulePath(data, module, callback) {
    ...
    if (/^\w+:/.test(module)) {
      ...
      while (dir) {
        ...
        moduleRootPath = path.join(root, dir);
