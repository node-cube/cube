...
const resolveExports = require('resolve.exports').resolve;

function findNearestPackageJson(dir, root) {
  let current = dir;
  while (true) {
...
function getSubpath(modPath, pkgRoot) {
  let rel = path.relative(pkgRoot, modPath).replace(/\\/g, '/'); // 兼容 Windows
...
function testModPathBaisc(cube, modPath, modName, modRootPath, isDirectory) {
  ...
  let tmpModPath;
  // @ns/module module 都需要在路径上拼 node_modules 
  const rootPath = modRootPath;
  try {
    let pkg;
    let isRoot = false;
    // 获取 package.json
    ...
    if (moduleMap && moduleMap[modName]) {
      ...
      debug('testModPath: try moduleMap', tmpModPath);
      try {
    ...
    // 新增：优先支持 package.json 的 exports 字段
    if (!alreadyFound && pkg && pkg.exports) {
      const subpath = getSubpath(modPath, modRootPath);
      // 使用 resolve.exports 解析 exports 字段
      try {
        let exportTarget = resolveExports(pkg, subpath, { require: true });
        if (Array.isArray(exportTarget)) {
          exportTarget = exportTarget[0];
        }
        if (exportTarget) {
          tmpModPath = path.join(modRootPath, exportTarget);
          debug('testModPath: try package.json.exports (via resolve.exports)', tmpModPath);
          tmpModPath = tryFiles(cube, root, tmpModPath);
          alreadyFound = true;
        }
      } catch (e) {
     ...
     */
    if (!alreadyFound && pkg && pkg.browserify && typeof pkg.browserify === 'string') {
     ...
     * "browser": "browser.js"
     */
    if (!alreadyFound && pkg && pkg.browser && typeof pkg.browser === 'string') {
    ...
    /**
     * browser 为 obj 的 2 种情况, 先判断是否存在，然后获取 broswer path
     * "browser": {
     *    "./aaa/aaa.js": "./aaa/dist/aaa.js",
     *    "fs": false
     * }
     */
    if (!alreadyFound && pkg && pkg.browser && typeof pkg.browser === 'object' && !!exist(pkg.browser, modPath, rootPath)) {
      const browserArr = Object.keys(pkg.browser);
      for (let i =0; i < browserArr.length; i++) {
        if (modPath === path.resolve(rootPath, browserArr[i])) {
          if (browser[browserArr[i]]) {
            // browser[key] 找到了，既拼接返回
            tmpModPath = path.resolve(rootPath, browser[browserArr[i]])
          } else {
            // browser[key] 被置为了 false, 需要忽略，这边造一个伪文件
            const id = `./${uuid.v4()}.js`;
            browser[browserArr[i]] = id;
            // rootPath 文件夹可能不存在, 因为可能人为的拼了 node_modules
            try {
              fs.statSync(rootPath);
            } catch(e) {
              fs.mkdirSync(rootPath);
            }
            fs.writeFileSync(path.join(rootPath, id), 'module.exports = {};');
            tmpModPath = path.join(rootPath, id);
          }
          try {
            debug('testModPath: try package.json.browser object', tmpModPath);
            tmpModPath = tryFiles(cube, root, tmpModPath);
            alreadyFound = true;
          } catch (e) {
          }
     ...
     */
    if (!alreadyFound && pkg && isRoot && pkg.main && typeof pkg.main === 'string') {
      ...
      try {
        debug('testModPath: try modPath', modPath);
...
// 判断 searchObj 中是否有 isTure 
function exist(searchObj, modPath, rootPath) {
  const searchArr = Object.keys(searchObj);
  let isExist = false;

  for (let i =0; i < searchArr.length; i++) {
    if (modPath === path.resolve(rootPath, searchArr[i])) {
      isExist = true;
      break;
    }
  }
...
function genCode(cube, qpath, ast, compress) {
  if (compress) {
    // 原有 minify 配置项直传 uglifyJS 会报错
    let minifyOpt = cube.config.uglifyMinify || {};
    let opt = {};
    ...
    let out = babelGenerator(opt.ast, {
      quotes: 'single',
      comments: false,
      compact: true,
      minified: true,
    ...
    out = uglifyJS.minify(out.code, minifyOpt);
    return out.code;
  } else {
    if (typeof ast === 'string') {
      return ast;
  ...
  resolveModulePath(data, module, callback) {
    ...
    if (/^\w+:/.test(module)) {
      ...
      while (dir) {
        ...
        moduleRootPath = findNearestPackageJson(nodeModulePath, root);
      ...
      try {
        p = testModPath(this, p, module, findNearestPackageJson(p, root));
      } catch (e) {
