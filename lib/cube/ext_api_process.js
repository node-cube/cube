  ...
  if (v.indexOf('/') === 0) { // abs path
    ...
    while (count >= 0) {
      ...
      if (count === 0) {
        ...
        console.error('[CUBE_WARN] max try in fixLeftPart()');
...
module.exports = {
  ...
  resolveModulePath(data, module, callback) {
    ...
    if (/^\w+:/.test(module)) {
      ...
      try {
        ...
        console.log(`[CUBE_WARN] required path:'${module}' not found in file: ${curModule}`);
        p = module;
        STATE = 1;
      }
    }
    ...
    */
    ...
    estraverse.traverse(ast, {
      enter: function (node) {
        ...
        let offset = args0.range[1] - args0.range[0] - 2;
        wait ++;
  ...
  wrapScript(data, callback) {
    ...
    if (flagCompress) {
      if (!data.wrap) {
        data.code = compressedCode(finalQpath, data.ast)[0];
      } else {
