...
module.exports = {
  ...
  transform(queryPath, callback) {
    async.waterfall([
       ...
       * 创建新文件对象
       */
      this.initFile.bind(this, queryPath),
      /**
       * 检查缓存
       */
   ...
   */
  initFile(qpath, callback) {
    debug('transform::initfile', qpath);
    // TODO qpath = /../../../ 处理路径中的../
    let data = {
    ...
    if (!type || !ext) {
      ...
      return callback({code: 'STATIC_FILE', message: msg});
    }
    data.type = type;
    data.ext = ext;
  ...
  checkCache(data, callback) {
    debug('transform::checkCache')
    let config = this.config;
    if (!config.cache) {
      return callback(null, data);
    }
    let cachePath = data.queryPath + ':' + (config.remote || '-') + '.json';
    let cacheData = this.caches.get(cachePath);
    if (!cacheData) {
      return callback(null, data);
    }
    fs.lstat(cacheData.absPath,  (err, stats) => {
      if (err) {
        return callback({
      ...
      if (cacheData.modifyTime === mtime) { // target the cache, just return
        ...
        return callback({code: 'CACHED'}, cacheData);
      }
  ...
  seekFile(data, callback) {
    ...
    debug('seekFile, type: %s ,qpath: %s, possibleExts: %s', data.type, qpath, possibleExts);
  ...
  readFile(data, callback) {
    ...
    fs.readFile(absPath, (err, content) => {
      if (err) {
        ...
        let type = this.extMap[data.ext];
        if (type === 'image') {
          data.code = data.source = content;
        } else {
  ...
  processFile(data, callback) {
    ...
    processActions.push(function (data, done) {
      debug('custom processors all done');
      switch(data.type) {
        case 'script':
  ...
  setCache(data, callback) {
    ...
    if (config.cache) {
      let cachePath = data.queryPath + ':' + (config.remote || '-') + '.json';
      this.caches.set(cachePath, data);
