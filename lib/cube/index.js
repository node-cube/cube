...
const EXT_API_PROCESS = require('./mixin_api_process');
const EXT_API_TRANSFER = require('./mixin_api_transform');
...
class Cube extends Events {
   ...
   *     - resPath {String} [optional] the http path for resource, same as resBase
   *     - entry {Array} 入口文件
  ...
  constructor(config) {
    ...
    if (!config.moduleRegistered) {
      config.moduleRegistered = {};
    }
    if (!config.processors) {
      config.processors = {};
    }
    config.resBase = config.resPath;
