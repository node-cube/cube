...
class Cube extends Events {
   ...
   *     - mangleFileNameIgnore {Array} 保留的文件名
   *     - built {Boolean}
  ...
  constructor(config) {
    ...
    this.fileNameMangle = new FileNameMangle(config);

    debug('loading default process');

    let self = this;
    let customProcessors = config.processors || {};
    /**
    ...
    try {
      ...
      if (!cfg || Object.keys(cfg).length <= 0) {
        console.error('==============');
        console.error('[WARN] package.json is not contain cube config， using default config');
        console.error('==============');
      }
    } catch (e) {
      console.error('==============');
      console.error(`[WARN] ${pkgPath} not found`);
      console.error('==============');
    }
      ...
      switch (key) {
        case 'processors':
          Object.keys(prop).forEach(function (ext) {
            if (!customProcessors[ext]) {
              customProcessors[ext] = prop[ext];
            }
          });
          break;
        case 'build':
     ...
     * 合并moduleMap
     *
    let moduleAlias = require('cube-module-alias');
    if (!config.moduleMap) {
      config.moduleMap = moduleAlias;
    } else {
      Object.keys(moduleAlias).forEach(function (key) {
        config.moduleMap[key] = moduleAlias[key];
      });
    }
    */

    Object.keys(defaultProcessConfig).forEach(function (key) {
      if (!customProcessors[key]) {
        customProcessors[key] = defaultProcessConfig[key];
      }
    });

    // register processors
    Object.keys(customProcessors).forEach(function (ext) {
      let plist = customProcessors[ext];
  ...
  register(ext, ps) {
    ...
    preparedProcessor.processors.forEach(p => {
      if (Array.isArray(p)) {
        processInstances.push(new p[0](p[1]));
      } else {
  ...
  */
  ...
  printFileShortNameMap() {
    console.log(this.fileNameGenerator.fileNameMaps);
  }
  /**
   * 获取 mangle之后，文件名的映射集合
   */
  getFileShortNameMap() {
    return this.fileNameGenerator.fileNameMaps;
  }
  /**
   * mangleFileName
  ...
  mangleFileName(fileName) {
    let mangleFileNameIgnore = this.config.mangleFileNameIgnore;
    if (mangleFileNameIgnore && mangleFileNameIgnore.indexOf(fileName) >= 0) {
      return fileName;
    }
    return this.fileNameMangle.mangle(fileName);
  }
}

mixin(Cube, EXT_API_PROCESS);
mixin(Cube, EXT_API_TRANSFER);
