...
const buildinProcessors = {
  BUILDIN_JS: require(path.join(__dirname, '../processor/js')),
  BUILDIN_CSS: require(path.join(__dirname, '../processor/css')),
  BUILDIN_JSON: require(path.join(__dirname, '../processor/json')),
  BUILDIN_HTML: require(path.join(__dirname, '../processor/html')),
  BUILDIN_RAW: require(path.join(__dirname, '../processor/raw'))
};
...
class Cube extends Events {
   ...
   *     - base {String} [optional] the http base for resource, same as resBase
   *     - devCache {Boolean} default true
   *     - cacheDir {AbsPath} .cubecache文件路径
  ...
  constructor(config) {
    ...
    config.resBase = config.base || config.resBase;
    ...
    this.caches = new Cache({
      tmpDir: config.cacheDir || path.join(config.root, './.cubecache'),
