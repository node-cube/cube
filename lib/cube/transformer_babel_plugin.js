...
const debug = require('debug')('cube:babel_transform');
...
function CubeTransformImportPlugin(babel, options) {
  const { types: t } = babel;
  let data = options.data;
  let cube = options.cube;
  // error
  let errors = data.errors;
  function setRequire(reqName, reqNameOrigin) {
    data.requires.push(reqName);
    data.requiresOrigin.push(reqNameOrigin);
  }
  function setAsync(reqName, reqNameOrigin) {
    data.asyncs.push(reqName);
    data.asyncsOrigin.push(reqNameOrigin);
  }
  ...
  function processRequireArgs(calleeName, arg0) {
    ...
    debug("resolve module path", modName)
    ...
    if (calleeName == 'require') {
      setRequire(modName, modNameOrign);
    } else if (calleeName == 'async') {
      setAsync(modName, modNameOrign);
    }
  ...
  let visitor = {
    ...
    CallExpression(path) {
      ...
      processRequireArgs(callFuncName, arg0);
    },
    /**
    ...
    ExportDeclaration(path) {
      ...
      path.replaceWith(genAst.genExportStmt(varList));
    },
    /**
     * export default decl
     * -- Default exports
...
exports.CubeTransformerPreset = function CubeTransformerPreset(api, options) {
  return {
    plugins: [
      [CubeTransformImportPlugin, options]
    ]
  }
}

exports.CubeTransformImportPlugin = CubeTransformImportPlugin;

// 最后一个babel处理器，将 requre的信息处理成 cube wraper
exports.CubeFinalProcessPlugin = function CubeFinalProcessPlugin(babel, options) {
  let importVisitor = CubeTransformImportPlugin(babel, options).visitor;
  return {
    visitor: {
      Program: {
        enter(path) {
          // console.log('cube import plugin, program enter');
        },
        exit(path) {
          topLevelPaths = path.get("body")
          for (const topPath of topLevelPaths) {
            if (topPath.isImportDeclaration()) {
              importVisitor.ImportDeclaration(topPath)
              console.log(">>>>>>>>>>>>>", topPath.node)
            }
          }

          let data = options.data;
          let cube = options.cube;
          // path.traverse(visitorPost);
          data.extraRequiresOrigin.forEach((req) => {
            let modName = req[1]
            if (req[0] == 'require') {
              data.requires.push(modName)
            } else if (req[0] == 'async') {
              data.asyncs.push(modName)
            }
          })
          data.requires = unique(data.requires);
          data.requiresOrigin = unique(data.requiresOrigin);
          data.asyncs = unique(data.asyncs);
          data.asyncsOrigin = unique(data.asyncsOrigin);
          // print errors
          data.errors.forEach((err)=> {
            console.log('[Error]', err);
          });
          // do cube wrap
          let finalAst = genAst.genCubeWraper(data.queryPath, data.requires, path.node.body);
