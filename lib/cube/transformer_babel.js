...
const debug = require('debug')('cube:transformer_babel');
const {CubeTransformImportPlugin, CubeTransformerPreset, CubeFinalProcessPlugin} = require("./transformer_babel_plugin");
...
function customMerge(srcValue, objValue) {
  if (_.isArray(objValue)) {
    if (!srcValue) {
      return objValue;
    } 
    return srcValue.concat(objValue); // 直接覆盖数组
  }
  return objValue;
}
...
module.exports = function (cube) {
  ...
  let workspaceRoot = cube.config.root;
  opt.presets && opt.presets.forEach((preset, i, a) => {
    ...
    if (typeof mod === 'string') {
      if (mod.startsWith(".")) {
        mod = path.join(workspaceRoot, mod);
      } if (/^[a-z@]/.test(mod)) {
        try {
  ...
  // console.log('>>>', opt, cube.config.babel);

  return {
    transform (data, callback) {
      let optTmp = _.cloneDeep(opt);
      /**
       * 注入 
       */
      [optTmp.plugins, optTmp.presets].forEach((ps) => {
        ps.forEach((p) => {
          if (Array.isArray(p) && p.length === 2 && p[1]['cube-plugin']) {
            p[1].data = data;
            p[1].cube = cube;
          }
        })
      ...
      // import transform
      optTmp.presets.push([
        CubeTransformerPreset, {cube, data}
      ]);
      // final
      optTmp.plugins.push([
        CubeFinalProcessPlugin, {cube, data}
      ]);

      optTmp.filename = data.queryPath;
      /*
      console.log('>>>', JSON.stringify(optTmp, (key, value) => {
        if (key === 'cube') {
          return "cube-cyclic";
        }
        return value;
