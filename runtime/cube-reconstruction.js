...
function extractModuleInfo(path) {
    ...
    var modulePath = match[3] || ""; // 模块路径（默认为空字符串）
...
function mockClassialCube() {
  ...
  function getGlobalRegister(require) {
    ...
    if (registerMap[moduleName][modulePath]) return registerMap[moduleName][modulePath].module;
  ...
  Cube.register = function (module, exports, { matchType = 'version' } = {}) {
    ...
    if (matchType === 'function') {
      ...
      registerMap[moduleName][modulePath] = {
        require: module,
        matchType,
        match: new RegExp(`^datav:\/npm\/${moduleName}\/(?<version>\\d+\\.\\d+\\.\\d+(-[a-zA-Z0-9.-]+)?(\\+[a-zA-Z0-9.-]+)?)\/${modulePath}$`),
        module: installedModules[module],
        moduleName,
      };
    }
