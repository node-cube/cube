  ...
  var Cube = /** @class */ (function () {
    function Cube() {
      ...
      this.init = function (config) {
        ...
        _this.config.requestMethod =
          (_e = config.requestMethod) !== null && _e !== void 0 ? _e : _this.config.requestMethod;
        _this.config.fetchUndeclaredModule =
          (_f = config.fetchUndeclaredModule) !== null && _f !== void 0 ? _f : _this.config.fetchUndeclaredModule;
        _this.config.aggregateFetch =
          (_g = config.aggregateFetch) !== null && _g !== void 0 ? _g : _this.config.aggregateFetch;
      ...
      this.use = function (moduleName, refererOrCallback, callbackOrOmitFix, omitFixOrUndefined) {
        ...
        moduleNames.forEach(function (mName) {
          return _this._load(mName, moduleNames);
        });
      };
      /** 执行 cube 源码 即原 Cube(...) */
      ...
      this._triggerCallback = function (moduleName, module) {
          ...
          if (entry) {
            ...
            if (!module.fired) {
              module.dep.forEach(function (m) {
                ...
                if (subModule === null || subModule === void 0 ? void 0 : subModule.fired) return;
                next_1 = false;
            ...
            if (next_1) {
              // 考虑标记 unload 提速
              if (
                Object.values(entry.loadSource).every(function (i) {
                  return i;
                })
              ) {
        ...
        if (finishedEntry.length) {
          module.refer.entryDep = refDep.entryDep.filter(function (i) {
            return !finishedEntry.includes(i);
          });
        }
      };
      /** 执行回调函数 */
      ...
      this._triggerAllCallback = function () {
        if (_this.state.delayTrigger) return;
        // this.state.delayTrigger = setTimeout(() => {
        // this.state.delayTrigger = undefined;
        if (_this.state.pendingQueue.length) return;
        if (Object.keys(_this.state.aggregateLoading).length) return;
            ...
            if (_this.state.lostDepModule[moduleName]) {
              _this.state.lostDepModule[moduleName].forEach(function (name) {
                if (!module.dep.includes(name)) {
                  ...
                  console.warn(
                    'Cube module '.concat(moduleName, ' \u7F3A\u5931\u58F0\u660E\u4F9D\u8D56 ').concat(name)
                  );
                }
              });
      ...
      this.cache = function () {
        ...
        console.info(
          'unloaded:',
          Object.fromEntries(
            Object.entries(_this.state.installedModules).filter(function (m) {
              return !m[1].loaded;
            })
          )
        );
